...
def get_list(category_id, thread_id):
    sql = "SELECT M.content, U.name, M.time, M.id FROM messages M, users U WHERE category_id=:category_id AND thread_id=:thread_id AND user_id=U.id AND M.visibility=True ORDER BY M.id"
